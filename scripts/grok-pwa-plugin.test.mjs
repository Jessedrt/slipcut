...
/**
 * An empty workspace for tests that assert the *default* chrome.
 *
 * The real workspace may carry baked OG identity (src/lib/og/site.json and
 * public/og.jpg), which `injectGrokPwaHead` reads from `cwd`. Tests that expect
 * the fallback behaviour must not inherit it, or they only pass on a checkout
 * with no site name set.
 */
const isolatedCwd = () => mkdtempSync(join(tmpdir(), "grok-iso-"));

...
test("the auth schema ships outside the globbed directory", () => {
  // The app's own migrations live in migrations/ and are meant to auto-apply;
  // the invariant is only that the auth schema is *not* one of them.
  const migrationsDir = join(projectRoot(), "migrations");
  const pending = pendingMigrations(readdirSync(migrationsDir), []);
  assert.equal(
    pending.some((m) => m.name === AUTH_MIGRATION),
    false,
    "migrations/0001_auth.sql would auto-apply on every fresh database — it must stay in migrations/auth/",
  );

...
async function readChunk(picks: TicketPick[], brief: string): Promise<Reading> {
  ...
  const user = [
    brief ? `LIVE RESEARCH (may be incomplete):\n${brief}` : "",
  ...
  // Gemini-first for speed. Fallbacks only if empty.
  if (geminiReady()) {
    const answer = await withTimeout(geminiChat(SYSTEM, user, 16_000), 18_000);
    if (answer) {
      const reading = matchRows(picks, answer);
      if (reading?.size) return tagReading(reading, "gemini");
    }
  }
  if (seekaiReady()) {

...
function pick(id: number): TicketPick {
  return { id: `p-${id}`, sport: "football", league: "Premier League", home: `H${id}`, away: `A${id}`, market: "Double Chance", selection: "Home or Draw", odds: 1.5, kickoff: Date.now() + 3_600_000, sporty: { eventId: `e-${id}`, marketId: "10", outcomeId: "9" } };
}

function dependencies(overrides: Partial<BookDependencies> = {}): BookDependencies {
  return {
    refresh: async (picks) => ({ available: picks, unavailable: [] }),
    mint: async () => ({ shareCode: "REAL12", shareURL: "https://www.sportybet.com/ng/?shareCode=REAL12", unavailable: 0 }),
    ...overrides,
  };
}
...
describe("reviewed booking", () => {
  it("does not mint when an outcome expired", async () => {
    ...
    const result = await mintReviewedSlip([pick(1)], "ng", dependencies({
      refresh: async (picks) => ({ available: [], unavailable: [{ pick: picks[0]!, reason: "closed" }] }),
      mint: async () => { minted = true; return { error: "should not run" }; },
    }));
  ...
  it("returns remaining legs for partial unavailability", async () => {
    ...
    const result = await mintReviewedSlip(rows, "ng", dependencies({
      refresh: async () => ({ available: [rows[0]!], unavailable: [{ pick: rows[1]!, reason: "closed" }] }),
    }));
    assert.equal(result.ok, false);
  ...
  it("reports mint failure without inventing a code", async () => {
    const result = await mintReviewedSlip([pick(1)], "ng", dependencies({ mint: async () => ({ error: "upstream" }) }));
  ...
  it("returns a real-shaped mint mock and respects manual removal", async () => {
    const rows = [pick(1), pick(2)];
    let refreshed = 0;
    const result = await mintReviewedSlip([rows[1]!], "ng", dependencies({ refresh: async (picks) => { refreshed = picks.length; return { available: picks, unavailable: [] }; } }));

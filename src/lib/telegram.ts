...
• “Split this ticket into 2 slips”
• “Trim this code to around 20 odds”
• “Change all draw selections to under 2.5”
• “Read this screenshot and book it for me”
• “Check today’s games and book the best teams to score”
• “Cook 30 odds basketball”

How to get better results:
• Say exactly what you want changed
• Include your target odds if you want a safer ticket
• Send the code, screenshot, or link together with your instruction

Useful:
• /help — this message
• 2odds / Daily 2 odds
• Cook 20 odds football
• trim · trim to 10x
• split into 2`;
...
async function mintAndReply(chatId: number, picks: TicketPick[], title: string) {
  ...
  if (!selections.length) {
    ...
    await tg("sendMessage", {
      chat_id: chatId,
      parse_mode: "HTML",
      text: [esc(title), "Parsed games (no SportyBet IDs yet — cannot mint code):", "", ...lines].join("\n").slice(0, 3900),
    });
    return;
  }
...
async function analyzeThenMintAll(chatId: number, picks: TicketPick[], title: string) {
  try {
    ...
    await tg("sendMessage", {
      chat_id: chatId,
      text: `Predict · ${picks.length} games · keeping all legs for odds target (${kept}/${picks.length} above soft bar)`,
    });
  } catch (err) {
    console.error("analyzeThenMintAll:", err instanceof Error ? err.message : err);
  }
...
async function splitCode(chatId: number, code: string, parts: number) {
  ...
  const analysis = await analyzePicks(base, 40);
  const ordered = (
    analysis.kept?.length ? analysis.kept : analysis.picks?.length ? analysis.picks : base
  ) as TicketPick[];
...
async function changeCodeMarkets(chatId: number, code: string, instruction: string) {
  ...
  await tg("sendMessage", {
    chat_id: chatId,
    text: "Market change applied on paper. Reminting where SportyBet IDs still match…",
  });
  ...
  if (still.length) {
    ...
    await tg("sendMessage", {
      chat_id: chatId,
      text: [
        "Updated selections (need fresh SportyBet market IDs to book):",
        ...lines,
        "",
        "Tip: say “cook under 2.5 today” and I will book from live markets.",
      ].join("\n"),
    });
  }
}
...
async function handleTextTicket(chatId: number, text: string, title = "From paste") {
  ...
  await tg("sendMessage", {
    chat_id: chatId,
    text: `Read ${picks.length} games. Trying to book on SportyBet…`,
  });
  await mintAndReply(chatId, picks, title);
}
...
export async function handleTelegramUpdate(update: TgUpdate) {
  ...
  if (splitMatch || isCmd(raw, "split") || /^split\b/i.test(raw)) {
    ...
    if (!code) {
      await tg("sendMessage", {
        chat_id: chatId,
        text: "Paste a booking code, then say: split into 2",
      });
      return;
    }
  ...
  if (isCmd(raw, "trim") || isCmd(raw, "optimize") || /^(trim|optimize)\b/i.test(raw)) {
    ...
    if (!code) {
      await tg("sendMessage", {
        chat_id: chatId,
        text: "Paste a booking code first, then: trim · trim to 10x",
      });
      return;
    }
  ...
  if (/^change\b|change all|make all .* (under|over|home|away)/i.test(raw)) {
    ...
    if (!code) {
      await tg("sendMessage", {
        chat_id: chatId,
        text: "Paste a code, then: change all draw selections to under 2.5",
      });
      return;
    }
  ...
  if (/^convert\b|bet9ja|stake\.com|stake ticket/i.test(lower)) {
    await tg("sendMessage", {
      chat_id: chatId,
      text: "Send the ticket as text or screenshot. I will parse it and try to book matching legs on SportyBet (direct Bet9ja/Stake code load is not available).",
    });
    return;
  }
